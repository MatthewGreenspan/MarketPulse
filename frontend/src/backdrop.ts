...
interface Series {
    /** Normalised 0..1 values, oldest first. One extra point scrolls off-canvas. */
    values: number[];
    band: [number, number];
    opacity: number;
    width: number;
...
function draw(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, progress: number): void {
    ...
    for (const line of series) {
        ...
        const points: Point[] = line.values.map((value, index) => ({
...
export function startBackdrop(canvas: HTMLCanvasElement): void {
    ...
    const onResize = () => {
        resize(canvas, ctx);
        if (reduced.matches) draw(canvas, ctx, 0);
    };
